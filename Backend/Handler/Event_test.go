...
func TestGetJoineEvent(t *testing.T) {
	...
	joinEvent.UserId = utils.GenerateJWT(joinEvent.UserId)
	...
	// Create a request to pass to our handler. We don't have any query parameters for now, so we'll
	// pass 'nil' as the third parameter.
	req, err := http.NewRequest("POST", "/GetJoinEvent", bytes.NewBuffer(body))
	if err != nil {
		t.Fatal(err)
		return
	}
	...
	// We create a ResponseRecorder (which satisfies http.ResponseWriter) to record the response.
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(GetJoinEvent(db))
	...
	// Check the response body is what we expect.
	expected := "Event joined get successfully"
	bodyValue := make(map[string]any)
...
func TestGetDeclineEvent(t *testing.T) {
	...
	declineEvent.UserId = utils.GenerateJWT(declineEvent.UserId)

	body, err := json.Marshal(declineEvent)
	if err != nil {
		t.Fatalf("Erreur lors de la sérialisation du corps de la requête : %v", err)
		return
	}
	...
	// Create a request to pass to our handler. We don't have any query parameters for now, so we'll
	// pass 'nil' as the third parameter.
	req, err := http.NewRequest("POST", "/GetDeclineEvent", bytes.NewBuffer(body))
	if err != nil {
		t.Fatal(err)
		return
	}
	...
	// We create a ResponseRecorder (which satisfies http.ResponseWriter) to record the response.
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(GetDeclineEvent(db))
